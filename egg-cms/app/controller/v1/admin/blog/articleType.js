...
class Controller extends BaseController {
  ...
  async create() {
    ...
    const allTypes = await service[this.app.config.public].admin[
      this.modleName
    ][this.serviceName].findAll({
      userId: ctx.state.user.id,
    });
  ...
  async update() {
    ...
    allTypes.rows.forEach((item) => {
      if (
        item.key === ctx.request.body.key &&
        item.id !== ctx.request.body.id
      ) {
