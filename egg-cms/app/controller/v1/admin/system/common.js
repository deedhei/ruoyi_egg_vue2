...
class CommonController extends Controller {
  async login() {
    ...
    ].admin.system.common.login(query);
    if (!result) {
  ...
  async resetPwd() {
    ...
    ctx.request.body["newPassword"] = await ctx.genHash(
    ...
    const result = await service[
      this.app.config.public
    ].admin.system.user.resetPwd(query, id);
    if (result) {
  ...
  async updateUserImg() {
    ...
    const result = await service[
      this.app.config.public
    ].admin.system.user.updateUserImg(query, id);
    if (result) {
      ctx.returnBody(null, 100030);
    } else {
      ctx.returnBody(null, 100031, 500);
