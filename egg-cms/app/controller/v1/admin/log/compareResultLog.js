...
class Controller extends BaseController {
  constructor(...arg) {
    super(...arg);
    this.serviceName = "compareResultLog";
    this.modleName = "log";
  }
  // // 查询用户信息
  // async getInfo () {
  ...
  async create() {
    ...
    let query = {
      ...
      errorMsg: ctx.request.body["errorMsg"],
      liveData: ctx.request.body["liveData"],
      liveVideo: ctx.request.body["liveVideo"],
      liveErrorCode: ctx.request.body["liveErrorCode"],
      liveErrorMsg: ctx.request.body["liveErrorMsg"],
      bestFrame: ctx.request.body["bestFrame"],
      profileImage: ctx.request.body["profileImage"],
      compareErrorCode: ctx.request.body["compareErrorCode"],
      compareErrorMsg: ctx.request.body["compareErrorMsg"],
      sim: ctx.request.body["sim"],
      needChargeResult: ctx.request.body["needChargeResult"],
      cardInfoInputJson: ctx.request.body["cardInfoInputJson"],
      requestId: ctx.request.body["requestId"],
      sdkVerificationId: ctx.request.body["sdkVerificationId"],
      createTime: new Date(),
    };
    const result = await service[this.app.config.public].admin[this.modleName][
      this.serviceName
